#Kaip paleisti: python pranasyste3.py --n-start 1 --n-end 9 --n-step 0.1 --interval-ms 10
...
def animate_plot_3d(
    ...
    frames_n = np.arange(n_start, n_end + 0.5 * n_step, n_step, dtype=float).tolist()
    frames = list(enumerate(frames_n))
    p_mode = "measured P levels" if use_measured_p_levels else "uniform P grid"
    z_min = float(df[TARGET_COLUMN].min())
    z_max = float(df[TARGET_COLUMN].max())
    cmap_dark_blue = LinearSegmentedColormap.from_list(
        "dark_blue_white",
        ["#010101", "#06162a", "#0f3b61", "#7ecbff", "#f1f9ff"],
        N=256,
    )

    fig.patch.set_facecolor("black")

    def update(frame: tuple[int, float]) -> None:
        frame_idx, frame_n = frame
        ax.cla()
        ...
        ax.set_ylabel("P")
        ax.set_zlabel("Ra")
        ax.view_init(elev=24, azim=-88 + frame_idx * 0.35)
        ...
        if frame_data["has_actual"]:
            ...
            title_text = (
                f"Ra predicted surface (N={frame_n:.2f}, Matern 2.5 no ARD, {p_mode}) | "
                f"RMSE={frame_data['rmse']:.4f}"
            )
            legend = ax.legend(loc="upper left", facecolor="black", edgecolor="#7ecbff")
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Matplotlib animation of Ra predicted surface across continuous N values "
            "(with intermediate N between integers)."
        )
    )
    parser.add_argument("--file", type=str, default="surikiuoti_duomenys_Nscan_3.xlsx")
    parser.add_argument("--n-start", type=float, default=1.0)
    parser.add_argument("--n-end", type=float, default=9.0)
    parser.add_argument(
        "--n-step",
        type=float,
        default=0.25,
        help="Step size for continuous N frames (smaller = smoother transition).",
    )
    ...
    animate_plot_3d(
        ...
        preprocessor=preprocessor,
        model=model,
        n_start=float(args.n_start),
        n_end=float(args.n_end),
        n_step=float(args.n_step),
