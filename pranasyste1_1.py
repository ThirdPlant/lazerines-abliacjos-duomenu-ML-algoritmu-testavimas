...
# Fixed model parameters (Matern 2.5 ARD)
USE_LOG_TARGET = True
C_VALUE = 0.42638329674243786
LS_N = 3.684816348935756
LS_P = 8.825687203522195
LS_F0 = 100.0
NOISE_LEVEL = 1e-08
...
def train_model(df: pd.DataFrame) -> tuple[ColumnTransformer, GaussianProcessRegressor]:
    ...
    preprocessor = ColumnTransformer([("scale", StandardScaler(), INPUT_COLUMNS)], remainder="drop")
...
def make_plot_3d(
    ...
    ax.set_title(
        f"Ra predicted grid for N={n_value:g} (Matern 2.5 ARD, scale_all+log_target, {p_mode})"
    )
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description="Interactive 3D plot: F0 (x), P (y), Predicted Ra (z) for fixed N."
    )
