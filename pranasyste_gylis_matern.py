#Kaip paleisti: python pranasyste_gylis_matern.py --n-start 1 --n-end 9 --interval-ms 700
import argparse
...
def train_model(df: pd.DataFrame) -> tuple[ColumnTransformer, GaussianProcessRegressor]:
    ...
    if USE_LOG_TARGET and np.any(y <= -1.0):
        raise ValueError(
            "Cannot use log1p target transform because Gylis contains values <= -1."
        )
...
def animate_plot_3d(
    ...
    frames = np.arange(n_start, n_end + 0.5 * n_step, n_step, dtype=float).tolist()
    p_mode = "measured P levels only"

    def update(frame_n: float) -> None:
        ax.cla()
        ...
        if frame_data["has_actual"]:
            ...
            ax.set_title(
                f"Depth predicted surface (N={frame_n:.2f}, Matern 2.5 ARD, {p_mode}) | "
...
def main() -> None:
    parser = argparse.ArgumentParser(
        description=(
            "Matplotlib animation of Gylis predicted surface across continuous N values."
        )
    )
